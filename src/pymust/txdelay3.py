...
def txdelay3Plane(param: utils.Param, tiltx: float, tilty: float):
    return txdelay3(param, tiltx, tilty)

def txdelay3Diverging(param: utils.Param, tiltx: float, tilty: float, omega: float):
    return txdelay3(param, tiltx, tilty, omega)

def txdelay3Focused(param: utils.Param, x, y, z):
...
def txdelay3(*args):
    ...
    if option == 'Plane Wave':
        ...
        tiltx = args[1] # tiltx = backend.array(args[1]).reshape((-1, 1))
        tilty = args[2] # tilty = backend.array(args[2]).reshape((-1, 1))
        assert backend.isscalar(tiltx+tilty), 'TILTx and TILTy must be two scalars.'
        assert backend.all(backend.abs(backend.array([tiltx,tilty]))<backend.pi/2), 'The tilt angles must verify |TILTx| and |TILTy| < pi/2'
        delays = (xe*backend.sin(tilty)-ye*backend.sin(tiltx))/c
    #-----
    elif option == 'Diverging Wave':
        # DR : problems checking if it is not a vector, used as a number later, no need for casting into array
        tiltx = args[1] # tiltx = backend.array(args[1]).reshape((-1, 1))
        tilty = args[2] # tilty = backend.array(args[2]).reshape((-1, 1))
        omega = args[3] # omega = backend.array(args[3]).reshape((-1, 1))
        assert backend.isscalar(tiltx+tilty+omega), 'TILTX, TILTY, and OMEGA must be scalar.'
        assert backend.all(backend.abs(backend.array([tiltx,tilty]))<backend.pi/2), 'The tilt angles must verify |TILTx| and |TILTy| < pi/2'
        assert omega<=2*backend.pi, 'The solid angle must be smaller than 2pi'
        assert omega>=0, 'The solid angle must be nonnegative'

        # check if the elements are on a plaid grid
        uxe = backend.unique(xe)
        uye = backend.unique(ye)
        [xep,yep] = backend.meshgrid(uxe,uye)
        test = backend.all(backend.sort(xep.flatten())==backend.sort(xe)) & backend.all(backend.sort(yep.flatten())==backend.sort(ye))
        ...
        x = -backend.sin(tilty)*backend.cos(tiltx)
        y = backend.sin(tiltx)
        z = -backend.cos(tilty)*backend.cos(tiltx)

        # corresponding azimuth and elevation
        # [az,el] = cart2sph(x,y,z);
        az = backend.arctan2(y,x)
        el = backend.arctan2(z,backend.sqrt(x**2 + y**2))

        # dimensions of the matrix array
        l = backend.max(xe)-backend.min(xe)+param.width # width of the matrix array
        b = backend.max(ye)-backend.min(ye)+param.height # height of the matrix array
        ...
        r = scipy.optimize.fminbound(myfun, 0, 2*backend.pi, xtol=1e-6) # DR : function tolerance not being taken into account.
        # r = scipy.optimize.minimize_scalar(myfun, bounds=(0, 2*backend.pi), method='bounded', options={'xatol': 1e-6, 'fatol': 1e-6}) # DR : alternative (not tried)

        # position of the virtual source, and TX delays
        cos_el = backend.cos(el)
        x0 = r*cos_el*backend.cos(az)
        y0 = r*cos_el*backend.sin(az)
        z0 = r*backend.sin(el)
    ...
    #-----
        ...
        y0 = backend.array(args[1]).reshape((-1, 1))
        z0 = backend.array(args[2]).reshape((-1, 1))
        assert len(x0)==len(y0)==len(z0), 'X0, Y0, and Z0 must have the same length.'
        if len(x0)==1: # focus point
            d = backend.sqrt((xe-x0)**2 + (ye-y0)**2 + z0**2)
        elif len(x0)==2: # focus line
            X = backend.concatenate((xe, ye, backend.zeros_like(xe)), axis=0)
            x1 = backend.tile(backend.concatenate((x0[0],y0[0],z0[0]), axis=0).reshape((-1,1)), (1, x0.shape[1]))
            x2 = backend.tile(backend.concatenate((x0[1],y0[1],z0[1]), axis=0).reshape((-1,1)), (1, x0.shape[1]))
            d = backend.linalg.norm(backend.cross(X-x1,X-x2, axis=0), axis=0)/backend.linalg.norm(x2-x1)
        else:
            ValueError('X0, Y0, and Z0 must have 1 or 2 elements.')
        delays = -d/c*backend.sign(z0)
...
def solidAngle(r, l, b, az, el):
    ...
    L1 = l/2 + r*backend.cos(el)*backend.cos(az)
    L2 = -l/2 + r*backend.cos(el)*backend.cos(az)
    B1 = b/2 + r*backend.cos(el)*backend.sin(az)
    B2 = b/2 - r*backend.cos(el)*backend.sin(az)
    H = r*backend.sin(el)

    # Solid angle calculation
    def w(l, b):
        return backend.arcsin(l*b/backend.hypot(l, H)/backend.hypot(b, H))
